...
\urlstyle{same}
\hypersetup{
  pdftitle={Mobile and Cyber-Physical Systems --- Appunti},
...
\item
  ...
  puramente informatico o puramente fisico?
\item
  Descrivere le quattro generazioni di Internet e le caratteristiche che
  differenziano l'IoT dalle generazioni precedenti.
\item
  Quali sono le differenze tra Edge Computing, Fog Computing e Cloud
  Computing in un'architettura IoT? In quale scenario conviene usare il
  Fog?
\item
  Quali funzionalità offre una piattaforma IoT e perché è necessario uno
  strato software intermedio tra dispositivi e applicazioni?
\item
  Quali sono le principali sfide di sicurezza e interoperabilità
  nell'IoT e come vengono affrontate a livello architetturale?
\end{itemize}
...
\item
  Quali sono le differenze tra Edge Computing, Fog Computing e Cloud
  Computing nell'IoT, e in quali scenari è preferibile ciascun
  approccio?
\item
  Come funziona il Federated Learning e quali sono i principali attacchi
  a cui è soggetto?
\item
  Perché i database NoSQL sono preferiti ai database relazionali nei
  contesti IoT? Quali vantaggi offre MongoDB?
\item
  Cosa si intende per vendor lock-in e vertical silos? Come il 5G e gli
  standard di interoperabilità cercano di ovviare a questi problemi?
\item
  Quali sono le principali vulnerabilità di sicurezza nei dispositivi
  IoT embedded e come vengono mitigate a livello di piattaforma?
\end{itemize}
...
\tightlist
\item
  Cosa si intende per vertical silo e vendor lock-in nell'IoT? Qual è la
  relazione tra i due concetti?
\item
  Quali sono le tre macro-categorie di scenari d'uso del 5G e quali
  requisiti tecnici le caratterizzano?
\item
  Descrivere le configurazioni di integrazione Tipo A, B, C e D: quando
  è necessario un gateway applicativo?
\item
  Quali sono i requisiti di sicurezza identificati dalla raccomandazione
  ITU-T Y.2066?
\item
  Qual è il ruolo del gateway nella sicurezza IoT e quali limitazioni
  incontrano i dispositivi vincolati?
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma di sequenza MQTT: Client A pubblica dati di temperatura, Client B li riceve tramite broker}]{images/MQTT_protocol_example_without_QoS.png}}
\emph{Fonte: Wikimedia Commons --- Il flusso completo: Client A si
connette (CONNECT/CONNACK), Client B si sottoscrive al topic
\texttt{temperature/roof} (SUBSCRIBE), Client A pubblica tre misurazioni
(20°C, 25°C, 38°C) con flag retain, il broker le inoltra a Client B.}
...
\tightlist
\item
  Quali sono i limiti dello stack TCP/IP tradizionale per l'IoT? Quali
  requisiti specifici impongono i dispositivi IoT a livello di rete e
  applicativo?
\item
  Descrivere il paradigma publish/subscribe e i tre tipi di
  disaccoppiamento che introduce rispetto al modello client/server.
\item
  Qual è il ruolo del broker MQTT? Quali operazioni svolge?
\item
  Quali sono le differenze tra i tre metodi di filtraggio dei messaggi
  (topic-based, content-based, type-based)?
\item
  Descrivere la struttura del pacchetto CONNECT e il significato di
  ciascun campo (Client ID, Clean Session, Will flags, KeepAlive).
\item
  Cosa contiene un pacchetto SUBSCRIBE e come risponde il broker con
  SUBACK?
\item
  Come funzionano le wildcard \texttt{+} e \texttt{\#} nei topic MQTT?
  Fornire esempi.
\item
  Perché si evita di usare il wildcard \texttt{\#} indiscriminatamente
  nelle sottoscrizioni?
\item
  Spiegare i tre livelli QoS di MQTT: garanzie offerte, meccanismi usati
  e casi d'uso ideali.
\item
  Descrivere il handshake a quattro fasi di QoS 2 (PUBLISH → PUBREC →
  PUBREL → PUBCOMP). Perché sono necessarie quattro fasi invece di due?
\item
  Il QoS tra publisher e broker può differire da quello tra broker e
  subscriber? Perché?
\item
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema del problema del terminale nascosto: A e C non si vedono ma collidono in B}]{images/Wifi_hidden_station_problem.png}}
\emph{Fonte: Wikimedia Commons --- I nodi A e C non si trovano nel
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma multipath: trasmettitore, percorso diretto e percorsi riflessi su ostacoli urbani}]{images/Multipath_propagation_diagram_en.png}}
\emph{Fonte: Wikimedia Commons --- Il trasmettitore (sinistra) invia il
segnale verso il ricevitore (destra) attraverso un percorso diretto e
...
\tightlist
\item
  Qual è la differenza tra una rete wireless single-hop e multi-hop con
  infrastruttura? Fare esempi concreti.
\item
  Perché il CSMA/CD non può essere applicato nelle reti wireless? Quale
  protocollo lo sostituisce e con quale logica?
\item
  Definire la Shannon Capacity e spiegare perché la capacità scala
  logaritmicamente con l'SNR e non linearmente.
\item
  Descrivere il problema del terminale nascosto: come si manifesta e
  perché il meccanismo di rilevamento delle collisioni classico non lo
  rileva?
\item
  Cosa si intende per Multipath e Coherence Time? Come la velocità del
  ricevitore influenza il \(T_c\)?
\item
  Spiegare il trade-off tra SNR e scelta dello schema di modulazione
  (es. BPSK vs QAM-256).
\item
  Cosa distingue la banda ISM dalla banda licenziata? Perché la banda
  2.4 GHz è particolarmente soggetta a interferenze?
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-7-lab-mobile-networks-01.png}}
\emph{Fig. --- I tre blocchi dell'architettura cellulare. La RAN
gestisce lo spettro, il backhaul è il ``trasporto'' verso l'interno
della rete, il Mobile Core fa da gateway verso Internet e custodisce le
funzioni di mobilità, autenticazione e charging.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-7-lab-mobile-networks-02.png}}
\emph{Fig. --- Topologia logica del 4G LTE. In rosso scuro le interfacce
di controllo (verso MME e HSS), il piano dati attraversa BS → S-GW →
P-GW → Internet.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-7-lab-mobile-networks-03.png}}
\emph{Fig. --- Separazione Control Plane / Data Plane in LTE. Il control
plane (sopra) trasporta segnalazione su SCTP fra BS, MME, HSS e gateway;
il data plane (sotto) trasporta i pacchetti utente in tunnel GTP-U
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Flusso GTP UE↔Internet con header IP/TEID}]{images/lezione-7-lab-mobile-networks-img-01.jpg}}
\emph{Fig. --- Composizione degli header GTP nel doppio senso del
traffico. \textbf{UE → Internet:} il pacchetto originale
\texttt{Src=UE,\ Dst=Internet} viene incapsulato dall'eNB in un
GTP+UDP+IP con \texttt{Src=eNB,\ Dst=S-GW,\ TEID=UL-S1-TEID} (tunnel
S1); l'S-GW lo ri-incapsula con
\texttt{Src=S-GW,\ Dst=P-GW,\ TEID=UL-S5-TEID} (tunnel S5); il P-GW
rimuove gli incapsulamenti e immette il datagramma originale su
Internet. \textbf{Internet → UE:} il P-GW riceve
\texttt{Src=Internet,\ Dst=UE}, lo incapsula con
\texttt{Src=P-GW,\ Dst=S-GW,\ TEID=DL-S5-TEID}; l'S-GW lo ri-incapsula
verso l'eNB con \texttt{Src=S-GW,\ Dst=eNB,\ TEID=DL-S1-TEID}; l'eNB
consegna il datagramma all'UE via radio.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-7-lab-mobile-networks-04.png}}
\emph{Fig. --- Stack del data plane LTE sul primo hop UE↔BS. Sopra l'IP
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura 5G User Plane con UPF e MEC}]{images/lezione-7-lab-mobile-networks-img-02.jpg}}
\emph{Fig. --- Architettura 5G User Plane (immagine NVIDIA/Mavenir). Il
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-7-lab-mobile-networks-05.png}}
\emph{Fig. --- Roaming in una rete globale di carrier interconnessi via
IPX. La SIM lega l'utente alla home network; quando è in roaming, la
rete visitata si appoggia all'HSS della home per autenticare e
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-7-lab-mobile-networks-06.png}}
\emph{Fig. --- Sequence diagram del protocollo AKA in 4G LTE. Le cinque
fasi (a)\ldots(e) corrispondono ai passi numerati delle slide.}
...
\tightlist
\item
  Descrivi i tre blocchi dell'architettura cellulare 4G/5G (RAN,
  Backhaul, Mobile Core) e le rispettive responsabilità.
\item
  A cosa servono MME, HSS, S-GW e P-GW nel 4G LTE? Quale di questi sta
  sul piano dati e quale su quello di controllo?
\item
  Spiega in dettaglio come il tunneling GTP supporti la mobilità senza
  interrompere le sessioni TCP attive.
\item
  Confronta il modello 4G EPC con il 5G NG-Core: quale ruolo del 4G
  corrisponde a quale Network Function 5G?
\item
  Cos'è la Service Based Architecture? Perché il 5G la adotta?
\item
  Descrivi i passi del protocollo AKA in 4G. Perché basta confrontare
  \(\text{res}_M\) con \(\text{xres}_{HSS}\)?
\item
  Quali sono le due differenze principali fra autenticazione 4G e 5G?
  Quale problema concreto risolvono?
\item
  Cosa distingue Standalone e Non-Standalone come opzioni di deployment
  5G?
\item
  Perché l'UPF abilita il Multi-Access Edge Computing?
\item
  Confronta brevemente Wi-Fi e 4G LTE dal punto di vista
  dell'autenticazione.
\end{itemize}
...
\tightlist
\item
  Cosa memorizza il broker e cosa memorizza il client in una sessione
  persistente con QoS 2?
\item
  Qual è la differenza tra sessione persistente e retained message? Sono
  meccanismi indipendenti?
\item
  In quali quattro circostanze il broker invia il messaggio di last
  will?
\item
  Come è strutturato il Fixed Header di un pacchetto MQTT? Cosa codifica
  il primo byte?
\item
  Perché PubSubClient non supporta QoS 2 e SSL/TLS? Qual è il vincolo
  architetturale?
\item
  Perché HTTP scala male per reti IoT con molti dispositivi rispetto a
  MQTT?
\item
  Quali sono i tre limiti strutturali di MQTT (broker, risorse, TCP)?
\item
  Cosa cambia nell'architettura client/server di CoAP rispetto al web
  tradizionale?
\item
  Perché CoAP usa UDP invece di TCP? Quali vantaggi porta e quali
  svantaggi introduce?
\item
...
\emph{Panoramica delle applicazioni ZigBee nei diversi domini.}
...
\emph{Corrispondenza tra i livelli ISO/OSI e gli standard IEEE 802.15.4
e ZigBee.}
...
\emph{Flusso di incapsulamento dei dati attraverso i livelli dello stack
ZigBee, da sorgente a destinazione.}
...
\tightlist
\item
  Quali sono i requisiti principali e le caratteristiche dello standard
  ZigBee? Come si relaziona con IEEE 802.15.4?
\item
  Descrivere le quattro Service Primitives e il ruolo di ciascuna nello
  stack protocollare.
\item
  Come avviene la Network Formation? Descrivere la sequenza di primitive
  coinvolte tra NWK e MAC.
\item
  Quali sono le differenze tra Join through Association e Direct Join?
\item
  Come funziona l'indirizzamento ad albero? Cosa sono \(R_m\), \(D_m\),
  \(L_m\)? Definire e applicare la formula \(C_{skip}\).
\item
  Data una rete con \(R_m\), \(D_m\), \(L_m\) noti, calcolare il range
  di indirizzi assegnato a un router che si aggrega come \(k\)-esimo
  figlio di un dato nodo a profondità \(d\).
\item
  Confrontare Tree routing e Mesh routing: meccanismi, vantaggi,
  limitazioni e compatibilità con il beaconing. Possono coesistere?
\item
  Come funziona il Route Discovery Protocol? Quali informazioni contiene
  la Route Discovery Table? Un nodo intermedio può rispondere all'RREQ?
\end{itemize}
...
\emph{Fig. --- Struttura della home network in un'architettura 4G/5G.}
...
\emph{Fig. --- Relazione tra home network e visited network in uno
scenario di roaming.}
...
\emph{Fig. --- Procedura di registrazione tra visited network e home
network.}
...
\emph{Fig. --- Sequenza di messaggi durante un handover tra due Base
Station in 4G.}
...
\tightlist
\item
  Qual è la differenza tra routing indiretto e routing diretto? Vantaggi
  e svantaggi di ciascuno.
\item
  Cos'è il triangle routing e quando si manifesta?
\item
  Quali tre protocolli sono necessari per implementare il routing
  indiretto?
\item
  Descrivi le quattro fasi della gestione della mobilità in 4G: quali
  entità sono coinvolte in ciascuna?
\item
  Qual è il ruolo di MME, HSS, S-GW e P-GW nell'architettura 4G?
\item
  Quali due tunnel vengono creati nella fase di configurazione del piano
  dati? Perché sono separati?
\item
  Come avviene un handover tra due Base Station? Descrivi i sette passi.
  Quale entità decide di avviarlo e quale sceglie la target BS?
\item
  Perché nel routing diretto un cambio di rete durante una sessione è
  più complesso che nel routing indiretto?
\item
  Perché le perdite radio degradano le prestazioni TCP nelle reti
  mobili?
\end{itemize}
...
\tightlist
\item
  Cos'è il SDN e quale problema risolve rispetto alle architetture
  tradizionali?
\item
  Qual è la differenza tra Data Plane (Forwarding) e Control Plane
  (Routing)?
\item
  Cos'è il traffico East-West e perché le architetture tradizionali
  faticano a gestirlo?
\item
  Descrivere i tre scenari di Traffic Engineering che risultano
  impossibili con il routing tradizionale.
\item
  Quali sono i tre limiti strutturali delle reti tradizionali
  (architettura statica, incoerenza delle policy, vendor lock-in)?
\item
  Descrivere i tre componenti dell'architettura SDN (data-plane
  switches, controller, network-control apps) e il ruolo di ciascuno.
\item
  Cosa significa che le network-control apps sono ``unbundled''? Qual è
  il vantaggio?
\item
  Cosa sono le Southbound e Northbound API in un'architettura SDN?
\item
  Cos'è l'astrazione match-action e perché è fondamentale per il data
  plane programmabile?
\item
  Perché il controller SDN è ``logicamente centralizzato'' ma
  fisicamente distribuito?
\item
  Qual è la differenza tra \(T_i = SPF(Topology, link\_weights)\) e
  \(\mathcal{F}: S(t) \rightarrow \{T_1, \ldots, T_n\}\)?
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/lezione-14-iot-design-img-01.jpg}}
\emph{Fig. --- Il confronto ``Intel vs Duracell'': le curve di
processore, HD e memoria crescono esponenzialmente nel tempo, mentre
quella della batteria rimane quasi orizzontale. Il gap energetico si
...
\pandocbounded{\includegraphics[keepaspectratio]{images/lezione-14-iot-design-img-02.jpg}}
\emph{Fig. --- Ripartizione del consumo energetico in un laptop: lo
schermo domina con il 48\%, seguito dal chipset (23\%), processore
(10\%), grafica (9\%), HD (6\%) e rete (4\%). In un sistema
general-purpose lo schermo è il primo target di ottimizzazione.}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/lezione-14-iot-design-img-03.jpg}}
\emph{Fig. --- Ripartizione del consumo energetico in un sensore
wireless: il Wireless NIC e il processore+chipset si equivalgono al 40\%
ciascuno, mentre sensing e ADC pesano il 20\%. Non c'è schermo, quindi
la radio diventa il componente critico.}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/lezione-14-iot-design-img-04.jpg}}
\emph{Fig. --- Diagramma stati/energia vs tempo per il codice di
esempio: il consumo istantaneo varia a scalini in base ai componenti
attivi. Il lungo periodo di idle (380 ms) abbassa drasticamente il
consumo medio.}
...
\pandocbounded{\includegraphics[keepaspectratio]{images/lezione-14-iot-design-img-05.jpg}}
\emph{Fig. --- Vita della batteria in mesi vs capacità (mA-hr) per due
...
\pandocbounded{\includegraphics[keepaspectratio]{images/lezione-14-iot-design-img-06.jpg}}
\emph{Fig. --- Vita del sensore (mesi) vs duty cycle per batterie da
2000 e 3000 mA-hr. La curva scende rapidamente: già passare dall'1\% al
3\% di DC dimezza approssimativamente il lifetime. L'effetto della
capacità della batteria diventa trascurabile ad alti DC.}
...
\item
  ...
  applicano all'IoT?
\item
  Perché la Legge di Moore non risolve automaticamente i problemi di
  design IoT nel breve termine?
\item
  Qual è la differenza nella distribuzione del consumo energetico tra un
  laptop e un sensore wireless?
\item
  Perché tenere la radio in modalità ``listen'' è quasi costoso quanto
  ricevere dati?
\item
  Come si calcola il duty cycle di un componente a partire dalla
  specifica comportamentale del firmware?
\item
  Dati i parametri hardware di un Mote, saper calcolare: duty cycle di
  ciascun componente, consumo orario in mAh, lifetime in ore/cicli.
\item
  Perché spegnere la radio è una ``decisione globale'' mentre spegnere
  il processore è ``locale''?
\item
  Qual è il trade-off tra edge processing e invio dei campioni grezzi al
  server in termini di lifetime?
\end{itemize}
...
\section{Approfondimento: Specifiche Reali di una Mote e Modelli di
DC}\label{approfondimento-specifiche-reali-di-una-mote-e-modelli-di-dc}

\subsection{Specifiche hardware della Mote-class
sensor}\label{specifiche-hardware-della-mote-class-sensor}

Per rendere concreto tutto il discorso teorico, le slide riportano le
specifiche misurate di un sensore di classe Mote basato su ATmega128L.
Questi numeri sono il riferimento per tutti gli esercizi numerici del
corso.

\pandocbounded{\includegraphics[keepaspectratio]{images/lezione-14-iot-design-img-07.jpg}}
\emph{Fig. --- Specifiche hardware complete di una Mote-class sensor:
consumo di corrente in ogni stato per microprocessore, radio (con
distinzione ricezione/trasmissione), logger (memoria flash) e sensor
board. La perdita di carica annua della batteria è del 3\%.}

Osservando la tabella emergono dettagli importanti: la radio in
ricezione (19,7 mA) consuma leggermente più di quella in trasmissione
(17,4 mA) --- un effetto già citato che spiega perché il solo ascoltare
sia così costoso. Il logger in scrittura (15 mA) ha un consumo
paragonabile alla radio, mentre in lettura scende a 4 mA e in sleep a
soli 2 μA. Il processore in sleep consuma appena 15 μA.

\subsection{Due modelli a confronto: DC 100\% vs DC
5\%}\label{due-modelli-a-confronto-dc-100-vs-dc-5}

\pandocbounded{\includegraphics[keepaspectratio]{images/lezione-14-iot-design-img-08.jpg}}
\emph{Fig. --- Tabella di confronto tra il modello con duty cycle al
100\% (tutto sempre acceso) e il modello con duty cycle al 5\%, con la
percentuale di tempo trascorsa in ciascuno stato per ogni subsistema. Il
logger mantiene un DC del 3\% in entrambi i modelli.}

Il confronto tra il modello a DC 100\% e quello al 5\% è illuminante.
Nel modello a pieno regime tutti i componenti sono sempre attivi. Nel
modello al 5\%, grazie alle chiamate \texttt{turnOn}/\texttt{turnOff},
il processore è attivo solo il 5\% del tempo, la radio il 5\% diviso tra
tx (1\%) e rx (4\%), e il sensor board appena l'1\%. Il logger mantiene
un proprio duty cycle del 3\% indipendente dagli altri, con parte delle
scritture e letture in parallelo ad altri componenti.

\pandocbounded{\includegraphics[keepaspectratio]{images/lezione-14-iot-design-img-09.jpg}}
\emph{Fig. --- Rappresentazione grafica degli stati per ciascun
subsistema nei due modelli (DC 100\% in alto, DC 5\% in basso). Nel
modello a DC 5\% i componenti trascorrono quasi tutto il periodo nello
stato idle (marrone scuro), con brevi finestre di attività (arancione).}

\subsection{Duty cycle: la flessibilità della ``forma'' del
ciclo}\label{duty-cycle-la-flessibilituxe0-della-forma-del-ciclo}

Un punto spesso sottovalutato è che il duty cycle specifica solo la
\textbf{frazione} di tempo attivo, non il suo posizionamento all'interno
del periodo. Lo stesso DC del 10\% può corrispondere a un burst di
attività all'inizio del periodo, uno alla fine, due burst distribuiti, o
qualsiasi altra configurazione temporale. Ciò che conta per il consumo
medio è la frazione totale.

\pandocbounded{\includegraphics[keepaspectratio]{images/lezione-14-iot-design-img-10.jpg}}
\emph{Fig. --- Esempi di un componente con DC al 10\%: la finestra di
attività (arancione) può posizionarsi in qualsiasi punto del periodo. Il
consumo medio è identico in tutti i casi; ciò che cambia è la latenza di
risposta e la sincronizzabilità con gli altri nodi.}

Questa flessibilità è sfruttata dai protocolli MAC dell'IoT per
sincronizzare le finestre di ascolto dei vari nodi: se tutti i vicini
sono svegli negli stessi istanti, la comunicazione è possibile anche con
DC molto bassi.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Esercizio Extra-1: Calcolo Completo con Attività
Separate}\label{esercizio-extra-1-calcolo-completo-con-attivituxe0-separate}

Questo esercizio è più realistico perché distingue tre fasi operative
separate --- campionamento, elaborazione e trasmissione --- ognuna con
la propria durata e i propri componenti attivi.

\textbf{Specifiche hardware}: ATmega128L (8 mA full, 0,015 mA sleep),
Radio (1 mA xmit, 0,02 mA sleep), Sensor Board (5 mA full, 0,005 mA
sleep), Batteria 2000 mAh.

\textbf{Comportamento del firmware} (periodo = 1/0,1 Hz = 10 s): -
Campionamento a 0,1 Hz → ogni 10 s, durata 0,5 ms → processore + sensore
attivi - Elaborazione dopo ogni campionamento, durata 2 ms → solo
processore attivo - Trasmissione dopo ogni elaborazione, durata 1 ms →
processore + radio attivi

\subsection{Calcolo dei duty cycle}\label{calcolo-dei-duty-cycle}

Il periodo è 10 secondi (frequenza 0,1 Hz). Le tre attività si svolgono
in sequenza nello stesso ciclo, quindi il tempo attivo totale per ciclo
è 0,5 + 2 + 1 = 3,5 ms su 10.000 ms.
...
Il sensor board è attivo solo durante il campionamento:
\(dc_\text{sensor} = 0,00005\). La radio è attiva solo durante la
trasmissione: \(dc_\text{radio} = 0,0001\).

\subsection{Calcolo del consumo per
ora}\label{calcolo-del-consumo-per-ora}

Applicando la formula
\(E = dc_\text{active} \cdot C_\text{active} + (1 - dc_\text{active}) \cdot C_\text{idle}\)
...
\textbf{Totale per ora}:
\(E_\text{tot} = 0,00525 + 0,01779 + 0,02010 \approx 0,0431\,\text{mAh}\)

\textbf{Lifetime}:
\(Lifetime = \frac{2000\,\text{mAh}}{0,0431\,\text{mAh/h}} \approx 46.358\,\text{ore} \approx 5,3\,\text{anni}\)

\begin{calloutnote}{Sintesi esercizio extra-1}

Grazie a un duty cycle di appena lo 0,035\% per il processore e ancora
meno per radio e sensore, il consumo orario è ridottissimo (0,043 mAh).
Con una batteria da 2000 mAh il lifetime teorico supera i 5 anni. Questo
mostra come applicazioni a bassa frequenza di campionamento (0,1 Hz)
permettano duty cycle estremi e vita utile pluriennale --- il paradigma
ideale per il monitoraggio ambientale o strutturale.

\end{calloutnote}

\begin{calloutwarning}{Attenzione alle semplificazioni}

In tutti gli esercizi si trascura il costo energetico del turn-on e
turn-off dei componenti, che in realtà ha un peso non nullo. Si trascura
anche l'autoscarica della batteria (3\% annuo per la Mote reale). Questi
effetti diventano rilevanti quando il DC è molto basso e il lifetime è
nell'ordine degli anni: la batteria potrebbe scaricarsi per autoscarica
prima che si esaurisca per consumo attivo.
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma del traffico East-West nei data center: percorsi statici e dinamici tra server, con connettività East-West come problema principale}]{images/lezione-15-lab-sdn-architecture-img-01.jpg}}
\emph{Fig. 0 --- Oltre il 70\% del traffico nei data center moderni è
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma di traffic engineering con routing tradizionale: topologia con nodi u, v, w, x, y, z e pesi sui link, evidenziando l'impossibilità di controllare percorsi specifici}]{images/lezione-15-lab-sdn-architecture-img-02.jpg}}
\emph{Fig. 0.1 --- Con il routing tradizionale, i link weights sono gli
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura SDN con controller remoto: il Remote Controller calcola e installa le forwarding table nei router, con control plane e data plane separati}]{images/lezione-15-lab-sdn-architecture-img-03.jpg}}
\emph{Fig. 0.2 --- Nel modello SDN, un Remote Controller logicamente
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura SDN a tre livelli: network-control applications (in alto), SDN Controller con northbound e southbound API (al centro), SDN-controlled switches (in basso)}]{images/lezione-15-lab-sdn-architecture-img-04.jpg}}
\emph{Fig. 0.3 --- L'architettura SDN è organizzata in tre livelli: le
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura SDN a tre livelli: Application Plane, Control Plane e Data Plane}]{images/lezione-15-lab-sdn-architecture-img-05.jpg}}
\emph{Fig. 1 --- L'architettura SDN stratificata: il Data Plane è il
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura interna di un OpenFlow Switch: porte, pipeline di flow table, group table, e canale OpenFlow verso il controller}]{images/lezione-15-lab-sdn-architecture-img-06.jpg}}
\emph{Fig. 2 --- L'OpenFlow Switch: il canale OpenFlow collega lo switch
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma di path computation: un pacchetto da A a B genera un Packet-In verso il controller, che risponde con FlowMod per popolare le flow table di S1 e S2}]{images/lezione-15-lab-sdn-architecture-img-07.jpg}}
\emph{Fig. 3 --- Path computation: le flow table degli switch sono
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Switch Initialization: handshake tra switch e controller con Feature Request e Feature Reply}]{images/lezione-15-lab-sdn-architecture-img-08.jpg}}
\emph{Fig. 4 --- Switch Initialization: al boot, lo switch stabilisce
una connessione TCP con il controller. Lo scambio Feature Request /
Feature Reply comunica al controller i parametri dello switch (Switch
ID, porte attive).}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Struttura del frame LLDP: campi Chassis ID, Port ID, Time to Live, End of LLDPDU}]{images/lezione-15-lab-sdn-architecture-img-09.jpg}}
\emph{Fig. 5 --- Struttura del frame LLDP. Il tipo Ethernet è 0x88cc. I
...
\tightlist
\item
  Come differisce il forwarding generalizzato dal destination-based
  forwarding? Quali campi possono essere usati per il match e quali
  azioni sono disponibili?
\item
  Descrivere la struttura di una flow entry OpenFlow (match, action,
  priority, counters) e fare un esempio di regola per implementare un
  firewall.
\item
  Descrivere le tre classi di messaggi OpenFlow (controller-to-switch,
  switch-to-controller, symmetric) e gli esempi principali per ciascuna.
\item
  Come funziona la flow table pipeline in OpenFlow? Cosa succede in caso
  di table-miss?
\item
  Descrivere il processo di topology discovery in una rete SDN: quale
  protocollo usa, quali messaggi vengono scambiati, e come il controller
  costruisce la mappa dei link.
\item
  Qual è la differenza tra Path Computation in SDN e routing distribuito
  tradizionale? Perché i FlowMod vengono inviati in ordine inverso?
\item
  Confrontare l'architettura di OpenDaylight (ODL) e ONOS: cosa li
  distingue in termini di struttura interna e approccio alla
  programmabilità?
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Mappa della rete B4 --- connessioni WAN tra i datacenter Google nel mondo}]{images/lezione-16-lab-sdn-applications-e-p4-language-img-01.jpg}}
\emph{Fig. --- La rete B4 connette una dozzina di siti (datacenter) in
tutto il mondo tramite link privati intercontinentali.}

\pandocbounded{\includegraphics[keepaspectratio,alt={Crescita del traffico B4 nel tempo}]{images/lezione-16-lab-sdn-applications-e-p4-language-img-02.jpg}}
\emph{Fig. --- Traffico B4 dal 2012 al 2015: crescita di 10x in 3.5
anni, con B4 che supera B2 per volume.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={SDN Architecture Stage 1 --- rete tradizionale distribuita}]{images/lezione-16-lab-sdn-applications-e-p4-language-img-03.jpg}}
\emph{Fig. --- Stage 1: la rete di partenza, con controllo distribuito
basato su BGP/ISIS. Un sito è un nodo WAN; un cluster è il dominio
server collegato a quel nodo.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={SDN Architecture Stage 2 --- deployment parziale di OpenFlow}]{images/lezione-16-lab-sdn-applications-e-p4-language-img-04.jpg}}
\emph{Fig. --- Stage 2: deployment graduale. I Site Controllers con
OpenFlow coesistono con i nodi di controllo tradizionale. Paxos
garantisce la fault tolerance del controller.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={SDN Architecture Stage 3 --- architettura target con Central TE Server}]{images/lezione-16-lab-sdn-applications-e-p4-language-img-05.jpg}}
\emph{Fig. --- Stage 3: rete target. Tutti i dispositivi sono
controllati via OpenFlow. Il Central TE Server fornisce ottimizzazione
globale del traffico.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura B4 completa con integrazione BGP e SDN}]{images/lezione-16-lab-sdn-applications-e-p4-language-img-06.jpg}}
\emph{Fig. --- L'architettura completa: BGP per la raggiungibilità, TE
Server centralizzato per l'ottimizzazione del traffico. Gateway, Site
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura del Traffic Engineering in B4}]{images/lezione-16-lab-sdn-applications-e-p4-language-img-07.jpg}}
\emph{Fig. --- Il TE Server riceve Flow Groups e funzioni di banda dal
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Esempio di forwarding multipath in B4}]{images/lezione-16-lab-sdn-applications-e-p4-language-img-08.jpg}}
\emph{Fig. --- Esempio di forwarding multipath: i flussi verso
9.0.0.0/24 vengono distribuiti equamente tra due tunnel tramite hash
degli header del pacchetto.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Flusso di compilazione di un programma P4 verso il target}]{images/lezione-16-lab-sdn-applications-e-p4-language-img-09.jpg}}
\emph{Fig. --- Il programmatore scrive un P4 Program; il compilatore
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Le tre componenti principali di un programma P4: Parser, Match-Action Pipeline, Deparser}]{images/lezione-16-lab-sdn-applications-e-p4-language-img-10.jpg}}
\emph{Fig. --- Pipeline P4: il Parser estrae gli header (state machine),
la Match-Action Pipeline applica le regole, il Deparser ricostruisce il
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Service Function Chaining con SDN --- flusso del traffico attraverso la catena}]{images/lezione-16-lab-sdn-applications-e-p4-language-img-11.jpg}}
\emph{Fig. --- SDN permette la definizione dinamica di service chain: il
...
\tightlist
\item
  Perché Google ha sviluppato B4? Quali sono i limiti delle WAN
  tradizionali che B4 risolve?
\item
  Descrivere le tre categorie di traffico in B4 e come influenzano le
  scelte di traffic engineering.
\item
  Descrivere i tre stadi di migrazione verso SDN in B4. Perché BGP è
  stato mantenuto?
\item
  Come funziona il TE Server di B4? Cosa sono i Flow Group, i Tunnel e i
  Tunnel Group?
\item
  Quali sono i limiti di OpenFlow che hanno motivato lo sviluppo di P4?
\item
  Cosa descrive un programma P4? Quali sono le sue tre componenti
  principali?
\item
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema S-MAC: trasmissione nei periodi di ascolto}]{images/lezione-17-mac-protocols-img-01.jpg}}
\emph{Fig. --- Il frame Sync e il frame Data vengono trasmessi durante
il periodo di attività del ricevitore; i periodi di silenzio T separano
le finestre di ascolto.}

\pandocbounded{\includegraphics[keepaspectratio,alt={Consumo energetico aggregato S-MAC su percorso 10 hop}]{images/lezione-17-mac-protocols-img-02.jpg}}
\emph{Fig. --- Consumo energetico totale in funzione del periodo
inter-arrivo dei messaggi. La curva ``No sleep cycles'' è il
riferimento; il duty cycle al 10\% riduce drasticamente i consumi;
l'adaptive duty cycle ottimizza ulteriormente.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Latenza media per hop in S-MAC}]{images/lezione-17-mac-protocols-img-03.jpg}}
\emph{Fig. --- Latenza media per hop in funzione del numero di hop. Il
duty cycle al 10\% senza adaptive listen introduce latenze molto più
elevate rispetto alla modalità senza sleep e alla variante con adaptive
listen.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema temporale del preamble sampling B-MAC}]{images/lezione-17-mac-protocols-img-04.jpg}}
\emph{Fig. --- Il mittente trasmette un lungo preambolo seguito dal
frame. Il ricevitore esegue campionamenti brevi e periodici (preamble
sampling); quando rileva il preambolo, mantiene la radio accesa per
ricevere il frame.}

\pandocbounded{\includegraphics[keepaspectratio,alt={Profilo di corrente del radio Mica-Mote durante il ciclo LPL}]{images/lezione-17-mac-protocols-img-05.jpg}}
\emph{Fig. --- Profilo di corrente (mA) nel tempo (ms) durante un ciclo
di Low-Power Listening: sleep → init → startup → receive mode →
ricezione segnale → decoding → sleep. Le fasi (a)-(g) evidenziano i
picchi di consumo durante l'attivazione.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Confronto temporale X-MAC vs B-MAC}]{images/lezione-17-mac-protocols-img-06.jpg}}
\emph{Fig. --- In B-MAC il preambolo dura per tutta la finestra di sleep
del ricevitore. In X-MAC il preambolo è una sequenza di frame corti con
l'ID del destinatario: il ricevitore interrompe la trasmissione appena
si sveglia, riducendo tempo ed energia spesi.}

\pandocbounded{\includegraphics[keepaspectratio,alt={Duty cycle totale trasmittenti: X-MAC vs B-MAC al variare del numero di nodi}]{images/lezione-17-mac-protocols-img-07.jpg}}
\emph{Fig. --- Simulazione su rete a stella. X-MAC (curve in basso)
mantiene un duty cycle significativamente inferiore rispetto a B-MAC
(curve in alto) per tutti gli intervalli LPL e al crescere del numero di
trasmittenti.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma temporale BoX-MAC}]{images/lezione-17-mac-protocols-img-08.jpg}}
\emph{Fig. --- Il mittente ripete il frame più volte. Il ricevitore, al
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Routing multi-hop in BoX-MAC}]{images/lezione-17-mac-protocols-img-09.jpg}}
\emph{Fig. --- Tre nodi i1, i2, i3 in cascata. Il nodo i1 trasmette
...
\tightlist
\item
  Qual è il tradeoff fondamentale nei protocolli MAC per IoT e come i
  tre approcci (sincronizzazione, preamble sampling, polling) lo
  affrontano diversamente?
\item
  In S-MAC, perché la latenza in percorsi multi-hop può essere elevata?
  In quale condizione si riduce?
\item
  Perché in B-MAC il preambolo deve essere più lungo del periodo di
  sleep del ricevitore?
\item
  In B-MAC, calcola il duty cycle del preamble sampling se
  \(t_{check} = 4 \times 10^{-4}\) s e la frequenza di campionamento è 5
  Hz:
  \(DC_{check} = f_c \times t_{check} = 5 \times 4 \times 10^{-4} = 2 \times 10^{-3} = 0.2\%\).
\item
  Come X-MAC riduce il costo energetico del preambolo rispetto a B-MAC?
\item
  In BoX-MAC il preambolo è un frame dati ripetuto: quali vantaggi porta
  in termini di latenza e overhead?
\item
  Descrivi il meccanismo di polling in IEEE 802.15.4: cosa fa il master
  quando riceve un messaggio per uno slave?
\end{itemize}
...
\emph{Fig. --- Confronto di Bit Error Rate (BER) in funzione del SNR tra
IEEE 802.15.4 (zona ``More Robust''), Bluetooth e altri standard. Lo
standard mostra eccellenti prestazioni in ambienti a basso SNR.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Struttura della PPDU IEEE 802.15.4}]{images/lezione-18-the-ieee-802-15-4-standard-img-01.jpg}}
\emph{Fig. --- Struttura della PPDU: SHR (Synchronization Header), PHR
(PHY Header) e PHY Payload (PSDU). Il frame viene trasmesso da sinistra
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Topologia Star IEEE 802.15.4}]{images/lezione-18-the-ieee-802-15-4-standard-img-02.jpg}}
\emph{Fig. --- Topologia Star: il PAN coordinator (arancione) al centro;
FFD blu e RFD rossi collegati direttamente. Anche i router agiscono come
RFD in questa topologia.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Topologia Peer-to-Peer IEEE 802.15.4}]{images/lezione-18-the-ieee-802-15-4-standard-img-03.jpg}}
\emph{Fig. --- Topologia Peer-to-Peer: gli FFD (blu) si interconnettono
tra loro; gli RFD (rosso) sono end-device connessi ciascuno a un FFD. Il
PAN coordinator è in arancione.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Struttura del superframe CAP e CFP}]{images/lezione-18-the-ieee-802-15-4-standard-img-04.jpg}}
\emph{Fig. --- Struttura del superframe: il beacon apre il periodo
attivo, diviso in CAP (accesso CSMA-CA, arancione) e CFP opzionale con
GTS (verde). Segue il periodo inattivo.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma flusso MAC Data Service e protocollo di associazione ZigBee}]{images/lezione-18-the-ieee-802-15-4-standard-img-05.jpg}}
\emph{Fig. --- In alto: lato end-device del protocollo ASSOCIATE con i
passi della primitiva ASSOCIATE.request. In basso: come il Join Protocol
di ZigBee (NWK layer) si appoggia al SCAN + ASSOCIATE del MAC 802.15.4.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma di sequenza del protocollo ASSOCIATE IEEE 802.15.4}]{images/lezione-18-the-ieee-802-15-4-standard-img-06.jpg}}
\emph{Fig. --- Diagramma di sequenza completo del protocollo ASSOCIATE:
flusso tra NWK/MAC del dispositivo originatore e NWK/MAC del
coordinatore. Dopo il pre-defined waiting time, il dispositivo invia una
Data request per recuperare la risposta.}
...
\tightlist
\item
  Quali sono le bande di frequenza supportate da IEEE 802.15.4 e quali
  data rate offrono?
\item
  Qual è la differenza tra RFD e FFD?
\item
  Come funziona la struttura del superframe? Qual è la differenza tra
  CAP e CFP?
\item
  Perché il CAP non può essere eliminato anche quando è presente il CFP?
\item
  Descrivi il trasferimento di dati da coordinatore a end-device in una
  rete beacon-enabled.
\item
  Descrivi il protocollo di associazione dal lato end-device e dal lato
  coordinatore.
\item
  Quali servizi di sicurezza offre il MAC layer di IEEE 802.15.4?
\item
  Qual è la differenza tra accesso al canale beacon-enabled e non
  beacon-enabled?
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Pipeline Biologging: dalla raccolta dati alla classificazione delle attività}]{images/lezione-19-caso-studio-tartarughe-img-01.jpg}}
\emph{Fig. --- Il flusso completo: il dispositivo di biologging fornisce
serie temporali, che vengono analizzate (da un osservatore umano o da un
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Approccio innovativo: classificatore automatico on-board con trasmissione dei soli risultati}]{images/lezione-19-caso-studio-tartarughe-img-02.jpg}}
\emph{Fig. --- Il nuovo paradigma: un micro-processore a bassa potenza
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Flusso operativo di Tortoise@: riconoscimento dello scavo e notifica agli erpetologi}]{images/lezione-19-caso-studio-tartarughe-img-03.jpg}}
\emph{Fig. --- Il sistema Tortoise@: un dispositivo con classificatore
ML identifica l'attività di scavo, invia un messaggio agli erpetologi
che recuperano le uova e le portano in un centro di protezione.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema del design Tortoise@ per l'efficienza energetica: stato di macchina a stati finiti}]{images/lezione-19-caso-studio-tartarughe-img-04.jpg}}
\emph{Fig. --- Il design di Tortoise@ per l'efficienza energetica: la
macchina a stati usa sensori ambientali a bassa frequenza come gate per
attivare il campionamento dell'accelerometro, minimizzando il consumo
complessivo.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Segnali accelerometrici per le tre attività: scavo, camminata e alimentazione}]{images/lezione-19-caso-studio-tartarughe-img-05.jpg}}
\emph{Fig. --- I segnali dell'accelerometro (asse X) per le tre
attività: lo scavo (digging) mostra un pattern oscillatorio
caratteristico con ampiezza maggiore e periodicità regolare rispetto
alla camminata e all'alimentazione.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Risultati task asincrono: accuratezza e overhead di memoria per IDNN, CNN, ESN, SVM}]{images/lezione-19-caso-studio-tartarughe-img-06.jpg}}
\emph{Fig. --- Confronto tra modelli per il task asincrono: accuratezza
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Risultati task sincrono con ESN: accuratezza 93\% e overhead 0,1 KB}]{images/lezione-19-caso-studio-tartarughe-img-07.jpg}}
\emph{Fig. --- L'ESN nel task sincrono raggiunge il 93\% di accuratezza
con soli 0,1 KB di overhead --- risultato notevole per un modello in
streaming.}
...
\tightlist
\item
  Qual è la differenza tra biotelemetria e bio-logging? Quando conviene
  l'uno e quando l'altro?
\item
  Perché l'approccio con classificatore on-board è superiore a quello
  con trasmissione delle serie temporali grezze?
\item
  Come funziona la macchina a stati di Tortoise@ per ottimizzare
  l'efficienza energetica?
\item
  Confronta le performance di IDNN, CNN, ESN e SVM nel task asincrono.
  Quale sceglieresti per un microcontrollore con 1 KB di RAM libera?
  Perché?
\item
  Quanto dato deve memorizzare il dispositivo Tortoise@ con elaborazione
  locale, rispetto all'approccio cloud? Calcola la differenza.
\item
  Cos'è un task sincrono vs asincrono nel contesto dell'activity
  recognition? Quale rete neurale supporta il task sincrono e perché?
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Confronto tra il modello tradizionale (appliance approach) e il modello virtualizzato (virtual appliance approach)}]{images/lezione-20-lab-network-function-virtualization-img-01.jpg}}
\emph{Fig. --- A sinistra, il modello tradizionale: ogni funzione (DPI,
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema di un servizio di rete come composizione di funzioni (Firewall, NAT, Load Balancer, Web Service)}]{images/lezione-20-lab-network-function-virtualization-img-02.jpg}}
\emph{Fig. --- Un servizio di rete minimo: il traffico dell'utente
attraversa in sequenza Firewall, NAT e Load Balancer prima di
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Esempio di rete NFV con service chaining su piattaforma SDN}]{images/lezione-20-lab-network-function-virtualization-img-03.jpg}}
\emph{Fig. --- Le reti fissa e mobile convergono su una piattaforma NFV.
Un service classifier instrada il traffico attraverso funzioni diverse
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Network Service in NFV: il Network Function Forwarding Graph con endpoint e VNF collegate da link logici}]{images/lezione-20-lab-network-function-virtualization-img-04.jpg}}
\emph{Fig. --- Un servizio end-to-end è un grafo: l'endpoint A è
connesso all'endpoint B attraverso VNF 1, VNF 2 e VNF 3 via link
logici.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={VNF-FG mappato su infrastruttura fisica: link logici e link fisici su tre reti di infrastruttura separate}]{images/lezione-20-lab-network-function-virtualization-img-05.jpg}}
\emph{Fig. --- Il grafo logico del servizio (VNF 1, VNF 2, VNF 3 con
link tratteggiati) è mappato su tre reti di infrastruttura fisiche
distinte. La separazione logico/fisico è il principio chiave.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={VNF Forwarding Graph con VNF-FG annidato (VNF-FG-2) e PoP fisici alla base}]{images/lezione-20-lab-network-function-virtualization-img-06.jpg}}
\emph{Fig. --- Il servizio end-to-end (tra End Point A e B) è un VNF-FG
...
\pandocbounded{\includegraphics[keepaspectratio,alt={NFV Reference Architectural Framework con NFVI, VNF, EMS, NFVO, VNFM, VIM e interfacce di riferimento}]{images/lezione-20-lab-network-function-virtualization-img-07.jpg}}
\emph{Fig. --- Il framework architetturale di riferimento NFV secondo
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema dei domini NFVI: compute domain, hypervisor domain e infrastructure network domain}]{images/lezione-20-lab-network-function-virtualization-img-08.jpg}}
\emph{Fig. --- L'NFVI si stratifica in tre livelli: hardware fisico
(compute, storage, network hardware), virtualizzazione
(hypervisor/virtualization layer) e risorse virtuali (virtual computing,
storage, network).}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Confronto architetturale tra container (Docker engine) e macchine virtuali (hypervisor)}]{images/lezione-20-lab-network-function-virtualization-img-09.jpg}}
\emph{Fig. --- A sinistra i container: condividono il kernel dell'host,
...
\tightlist
\item
  Che cos'è un middlebox e perché la loro proliferazione ha motivato
  NFV?
\item
  Qual è la differenza tra CAPEX e OPEX nel contesto NFV?
\item
  Descrivere il Network Function Forwarding Graph e la separazione tra
  definizione logica e deployment fisico.
\item
  Quali sono i tre componenti principali dell'architettura NFV-MANO?
  Descrivere le responsabilità di NFVO, VNFM e VIM.
\item
  In cosa consiste il problema del VNF chain placement e quali approcci
  risolutivi esistono?
\item
  Confrontare container e VM come opzioni di virtualizzazione per le
  VNF.
\item
  Cosa sono gli NFVI-PoP e come si differenziano NFVI-PoP network e
  transport network?
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Sito ufficiale di Mininet con navigazione e documentazione}]{images/lezione-21-lab-network-emulator-con-comnetsemu-img-01.jpg}}
\emph{Fig. --- Il sito ufficiale di Mininet (http://mininet.org/) con la
sua documentazione, walkthrough e API di riferimento.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema della SimpleSwitchTopology con uno switch centrale e N host foglia}]{images/lezione-21-lab-network-emulator-con-comnetsemu-img-02.jpg}}
\emph{Fig. --- SimpleSwitchTopology: uno switch centrale connesso a N
host tramite link individuali.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema delle veth pairs con due host in namespace separati collegati tramite un software switch}]{images/lezione-21-lab-network-emulator-con-comnetsemu-img-03.jpg}}
\emph{Fig. --- Architettura delle veth pairs: Host 1 e Host 2 hanno
ciascuno il proprio network namespace con interfacce veth collegate
tramite un software switch nel Root Namespace.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma dei namespace con firefox e httpd, virtual Ethernet pairs e Software Switch}]{images/lezione-21-lab-network-emulator-con-comnetsemu-img-04.jpg}}
\emph{Fig. --- Il piano dati di Mininet: due namespace separati
collegati tramite virtual Ethernet pairs e un Software Switch nel Root
Namespace. Mininet gestisce automaticamente questa configurazione al
posto dell'utility \texttt{ip}.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura ComNetsEmu con server A, server B e client che comunicano tramite data plane e SDN controller}]{images/lezione-21-lab-network-emulator-con-comnetsemu-img-05.jpg}}
\emph{Fig. --- Architettura ComNetsEmu: i container rappresentano
applicazioni su host virtuali; il piano dati è gestito da virtual
switch; il control plane è affidato a un SDN controller.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Contenuto del file Dockerfile.dev\_test con la riga netcat da aggiungere evidenziata}]{images/lezione-21-lab-network-emulator-con-comnetsemu-img-06.jpg}}
\emph{Fig. --- Il file \texttt{Dockerfile.dev\_test}: aggiungere la riga
\texttt{netcat\ \textbackslash{}\textbackslash{}} prima di
\texttt{telnet\ \textbackslash{}\textbackslash{}} nella lista dei
pacchetti da installare con \texttt{apt-get}.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Topologia dell'echo server con h1 (bash) e h2 (echo svr) connessi tramite s1 e s2}]{images/lezione-21-lab-network-emulator-con-comnetsemu-img-07.jpg}}
\emph{Fig. --- Topologia dell'echo server: \texttt{h1} (10.0.0.1) con
container \texttt{bash} e \texttt{h2} (10.0.0.2) con container
\texttt{echo\_server}, collegati da due switch con link da 10 Mbps e
10ms di delay.}
...
\item
  Come vengono implementati gli host virtuali in Mininet? (network
  namespace)
\item
  Cosa sono le veth pairs e a cosa servono?
\item
  Come si configura il delay di un link virtuale in Mininet con
  \texttt{tc}?
\item
  Qual è il ruolo di Docker in ComNetsEmu rispetto al tipo di host di
  default di Mininet?
\item
  Cosa fa la classe \texttt{APPContainerManager} in ComNetsEmu?
\item
  Qual è la differenza tra \texttt{DockerHost} e un host Mininet
  tradizionale?
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Formula di Mathis per il throughput TCP}]{images/lezione-22-lab-multi-access-edge-computing-mec-img-01.jpg}}
\emph{Fig. --- La formula di Mathis: il throughput TCP è inversamente
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Confronto CDF di latenza: cloudlet vicino vs.~lontano per tre applicazioni}]{images/lezione-22-lab-multi-access-edge-computing-mec-img-02.jpg}}
\emph{Fig. --- CDF del tempo di risposta per Fluid Graphics, Face
Recognition e Augmented Reality: il cloudlet vicino (linea rossa
tratteggiata) mostra latenze significativamente inferiori.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema delle posizioni possibili dell'edge nella rete mobile}]{images/lezione-22-lab-multi-access-edge-computing-mec-img-03.jpg}}
\emph{Fig. --- Confronto tra il percorso dati UE→Cloud (verde,
attraverso core network, internet e data center) e UE→MEC Service
(arancione, terminato al Mobile Edge Cloud). Fonte: GIGABYTE, 2021.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Scenario MEC per l'analisi di flussi video}]{images/lezione-22-lab-multi-access-edge-computing-mec-img-04.jpg}}
\emph{Fig. --- Il MEC Server, co-locato con la base LTE, esegue video
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Scenario MEC come IoT gateway}]{images/lezione-22-lab-multi-access-edge-computing-mec-img-05.jpg}}
\emph{Fig. --- Un'applicazione IoT GW App gira sul MEC Server,
raccogliendo traffico da dispositivi IoT eterogenei via radio. Verso la
core network vengono propagate solo analisi aggregate e eventi
rilevanti.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura del MEC Framework con livelli di gestione}]{images/lezione-22-lab-multi-access-edge-computing-mec-img-06.jpg}}
\emph{Fig. --- Il MEC Framework: i MEC host ospitano la virtualization
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Tabella MTS: data rate e affidabilità al variare del numero di link N}]{images/lezione-22-lab-multi-access-edge-computing-mec-img-07.jpg}}
\emph{Fig. --- Con N=4 link: data rate aggregato 400 Mbps (split) oppure
...
\tightlist
\item
  Dato un insieme di link wireless con parametri di QoS, determinare
  quale operazione MTS è necessaria per soddisfare i requisiti di
  ciascuna applicazione.
\item
  Spiegare il trade-off tra split traffic e duplicate traffic con
  esempio numerico.
\item
  Perché nessuna singola tecnologia di accesso può soddisfare tutti i
  requisiti? Come risponde MEC?
\item
  Qual è la differenza tra MEC host, MEC platform e MEC application?
\item
  Quali informazioni espone la RNIS API e per quali ottimizzazioni viene
  usata?
\end{itemize}
...
\section{Implementazione e strumenti}\label{implementazione-e-strumenti}

MEC è supportato da tutti i principali cloud provider con soluzioni
proprietarie: \textbf{Azure private multi-access edge compute} (con
partner Nokia ed Ericsson), \textbf{AWS Wavelength} (che porta
l'infrastruttura AWS nei data center dei partner telco per soddisfare
requisiti di bassa latenza e residenza dei dati).

Per la sperimentazione, ETSI fornisce \textbf{MEC Sandbox}
(\texttt{try-mec.etsi.org}), un ambiente interattivo per imparare e
sperimentare con le MEC Service API senza necessità di infrastruttura
fisica.
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-23-embedded-programming-e-arduino-01.png}}
\emph{Fig. --- Pipeline di sviluppo per sistemi embedded: dal codice
sorgente all'eseguibile nel firmware.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-23-embedded-programming-e-arduino-02.png}}
\emph{Fig. --- Modello di esecuzione Arduino: singolo thread, nessuna
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-23-embedded-programming-e-arduino-03.png}}
\emph{Fig. --- Flusso di esecuzione TinyOS: eventi gestiti
immediatamente, elaborazione delegata ai task.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-23-embedded-programming-e-arduino-04.png}}
\emph{Fig. --- Modello a tre livelli: hardware genera interrupt, OS li
astrae in eventi, l'applicazione li gestisce con handler e task.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Scheda Arduino Uno con pinout annotato}]{images/lezione-23-embedded-programming-e-arduino-img-01.jpg}}
\emph{Fig. --- Arduino Uno: microcontrollore ATmega328 con connettori
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Schema circuito Arduino con interrupt: pulsante su pin 2 e LED su pin 7}]{images/lezione-23-embedded-programming-e-arduino-img-02.jpg}}
\emph{Fig. --- Circuito di esempio: pulsante sul pin 2 (INT0) con
resistore pull-down 10KΩ, LED verde sul pin 7 con resistore 220Ω.}
...
\item
  Descrivere le sfide principali della programmazione embedded (timing,
  affidabilità, memoria, energia).
\item
  Come funziona la cross-compilazione? Cosa contiene l'eseguibile
  finale?
\item
  Descrivere il modello di esecuzione di Arduino. Perché non usa thread
  multipli?
\item
...
\item
  Quali tipi di interrupt esistono in Arduino? Descrivere
  \texttt{attachInterrupt()} e le modalità di trigger.
\item
  Perché le variabili condivise tra loop e interrupt handler devono
  essere \texttt{volatile}?
\item
  Descrivere le modalità di sleep dell'ATmega328P e i loro trade-off.
\item
  Scrivere un frammento di codice che configuri INT0 su CHANGE e
  gestisca il risveglio da sleep con un interrupt esterno.
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-24-energy-harvesting-iot-01.png}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-24-energy-harvesting-iot-02.png}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Produzione solare giornaliera a Madrid e Amburgo con distribuzione oraria}]{images/lezione-24-energy-harvesting-iot-img-01.jpg}}
\emph{Fig. --- Produzione solare giornaliera (Wh/day) e irradianza
oraria a Madrid e Amburgo per le quattro stagioni. Madrid raggiunge
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Produzione ideale mensile del pannello KL-SUN3W a Madrid}]{images/lezione-24-energy-harvesting-iot-img-02.jpg}}
\emph{Fig. --- Produzione oraria stimata del pannello KL-SUN3W a Madrid,
mese per mese (0--24 h). I mesi estivi raggiungono \textasciitilde3 Wh
per slot orario; dicembre e gennaio sono quasi trascurabili.}

Nella realtà, la produzione effettiva diverge nettamente dalla stima
nelle giornate nuvolose:

\pandocbounded{\includegraphics[keepaspectratio,alt={Produzione solare reale vs stimata --- Ciudad Real, 27 marzo 2017}]{images/lezione-24-energy-harvesting-iot-img-03.jpg}}
\emph{Fig. --- Produzione reale (triangoli rossi) vs stimata EWMA
(quadrati blu), per slot da 10 minuti. Giornata molto nuvolosa: la
produzione reale è fortemente discontinua e inferiore alla stima.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Confronto produzione solare vs eolica su 12 giorni}]{images/lezione-24-energy-harvesting-iot-img-04.jpg}}
\emph{Fig. --- Potenza (watt) su 12 giorni: sopra, pannello solare
(picchi giornalieri regolari); sotto, turbina eolica (picchi irregolari,
anche notturni). Fonte: Sharma et al., IEEE Secon 2010.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Tensione a circuito aperto vs stato di carica per batterie Li-ion}]{images/lezione-24-energy-harvesting-iot-img-05.jpg}}
\emph{Fig. --- Tensione a circuito aperto (V) vs stato di carica (0--1)
per nove batterie Li-ion nuove. La relazione è quasi lineare tra 3.4 V e
4.2 V.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-24-energy-harvesting-iot-03.png}}
\emph{Fig. --- Mappa concettuale della lezione.}
...
\item
  ...
  In quale caso si applica ciascuna?
\item
  Scrivi le equazioni di conservazione dell'energia e di capacità finita
  per un buffer non ideale. Qual è la differenza tra condizione
  necessaria e sufficiente?
\item
  Classifica le sorgenti di energia per controllabilità e prevedibilità.
  Perché la prevedibilità è importante per la pianificazione?
\item
  Come si misura la carica di una batteria tramite ADC? Ricava la
  formula di conversione da campione ADC a carica in mAh.
\item
  Enuncia il teorema di Kansal: quali sono le tre condizioni sufficienti
  per la neutralità energetica?
\item
  Descrivi l'algoritmo di Kansal nei due casi (sovrapproduzione e
  sottoproduzione). Come funziona l'adattamento dinamico?
\item
  Come funziona il filtro EWMA per la previsione della produzione
  energetica solare?
\item
  Cos'è il modello a task? In cosa supera l'approccio di Kansal?
\item
  Qual è la complessità del problema di ottimizzazione a task? Come si
  risolve con la programmazione dinamica?
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Pipeline di conversione analogico-digitale: da segnale analogico a codifica binaria}]{images/lezione-25-lab-teoria-dei-segnali-introduzione-e-serie-di-fourier-img-01.jpg}}
\emph{Fig. --- Dalla sorgente analogica ai bit: campionamento
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Segnali a tempo continuo: ampiezza continua (curva liscia) vs ampiezza quantizzata (gradini)}]{images/lezione-25-lab-teoria-dei-segnali-introduzione-e-serie-di-fourier-img-02.jpg}}
\emph{Fig. --- In alto: segnale analogico, continuo in tempo e ampiezza.
In basso: segnale continuo in tempo ma quantizzato in ampiezza (segnale
a gradini).}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Segnali a tempo discreto: ampiezza continua (campioni a qualsiasi altezza) vs ampiezza quantizzata (campioni su livelli finiti)}]{images/lezione-25-lab-teoria-dei-segnali-introduzione-e-serie-di-fourier-img-03.jpg}}
\emph{Fig. --- In alto: segnale discreto con ampiezza continua. In
basso: segnale digitale, discreto sia nel tempo che nell'ampiezza.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Le componenti armoniche s\_0, s\_1, s\_2, s\_3, s\_4 si sommano per costruire il segnale s(t)}]{images/lezione-25-lab-teoria-dei-segnali-introduzione-e-serie-di-fourier-img-04.jpg}}
\emph{Fig. --- Un segnale si costruisce sommando componenti sinusoidali:
la componente costante (\(s_0\)), la fondamentale (\(s_1\)) e le
armoniche superiori (\(s_2, s_3, s_4\)). Cambiare i coefficienti cambia
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Fenomeno di Gibbs: con 20 armoniche (sinistra) e 50 armoniche (destra) il salto rimane evidente}]{images/lezione-25-lab-teoria-dei-segnali-introduzione-e-serie-di-fourier-img-05.jpg}}
\emph{Fig. --- Fenomeno di Gibbs: aggiungere più armoniche riduce le
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Convergenza della serie di Fourier all'onda rettangolare per k=1, k=2, k=100}]{images/lezione-25-lab-teoria-dei-segnali-introduzione-e-serie-di-fourier-img-06.jpg}}
\emph{Fig. --- Al crescere del numero di armoniche considerate (verde →
rosso → blu) la serie converge all'onda rettangolare. La forma in blu
con k=100 è quasi perfetta, con il solo fenomeno di Gibbs ai salti.}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura generica per il network slicing}]{images/lezione-26-lab-network-slicing-img-01.jpg}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Topologia ad anello con quattro switch e quattro host}]{images/lezione-26-lab-network-slicing-img-02.jpg}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Topologia con separazione Upper Slice e Lower Slice evidenziata}]{images/lezione-26-lab-network-slicing-img-03.jpg}}
\emph{Fig. --- Le due slice risultanti: la linea arancione separa
l'Upper Slice (S1-eth1/eth3 ↔ S2 ↔ S4-eth1/eth3) dalla Lower Slice
(S1-eth2/eth4 ↔ S3 ↔ S4-eth2/eth4).}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Architettura Ryu: Ryu app sopra il layer Ryu sopra l'OF Switch}]{images/lezione-26-lab-network-slicing-img-04.jpg}}
...
\tightlist
\item
  Quante coppie PACKET\_IN / PACKET\_OUT vengono scambiate durante un
  ping h1↔h3? Perché?
\item
  Qual è l'azione di default installata da
  \texttt{switch\_features\_handler}? A cosa serve?
\item
  Perché il controller non riceve tutti i pacchetti del ping, ma solo i
  primissimi?
\item
  Cosa succederebbe se si eliminassero le flow rule di S2 durante una
  comunicazione attiva?
\end{itemize}

\end{calloutquestion}

\begin{callouttip}{Risposta alla prima domanda}

Per il primo ping (ICMP Echo Request + Reply) ci sono \textbf{due
scambi} PACKET\_IN/PACKET\_OUT: uno per il pacchetto dal h1 verso h3
(che imposta la regola su S1, S2, S4 nel percorso di andata) e uno per
la risposta da h3 verso h1. I ping successivi non generano PACKET\_IN
perché le flow rule sono già installate.

\end{callouttip}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{Esercizio B: service chain con un
firewall}\label{esercizio-b-service-chain-con-un-firewall}

La \textbf{service chain} (o \emph{function chaining}) è il meccanismo
con cui il traffico viene deviato attraverso una sequenza di funzioni di
rete prima di raggiungere la destinazione. Nell'esercizio B si simula
l'inserimento di un firewall (S5) nel percorso dell'upper slice.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Topologia Esercizio B con switch S5 come firewall inline collegato a S2}]{images/lezione-26-lab-network-slicing-img-05.jpg}}
\caption{Topologia Esercizio B con switch S5 come firewall inline
collegato a S2}
\end{figure}

\emph{Fig. --- Topologia Esercizio B: S5 è inserito tra S2-eth3 e
S2-eth4, creando un percorso inline per filtrare il traffico dell'upper
slice.}

Il percorso fisico del pacchetto diventa: h1 → s1 → \textbf{s2-eth1} →
\textbf{s2-eth3} → \textbf{s5-eth1} → \textbf{s5-eth2} →
\textbf{s2-eth4} → \textbf{s2-eth2} → s4 → h3.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Modifica \texttt{network.py}}: aggiungere lo switch S5 e due
  link verso S2

\begin{Shaded}
\begin{Highlighting}[]
\VariableTok{self}\NormalTok{.addSwitch(}\StringTok{"s5"}\NormalTok{, }\OperatorTok{**}\NormalTok{\{}\StringTok{"dpid"}\NormalTok{: }\StringTok{"}\SpecialCharTok{\%016x}\StringTok{"} \OperatorTok{\%} \DecValTok{5}\NormalTok{\})}
\VariableTok{self}\NormalTok{.addLink(}\StringTok{"s2"}\NormalTok{, }\StringTok{"s5"}\NormalTok{)  }\CommentTok{\# s2{-}eth3 ↔ s5{-}eth1}
\VariableTok{self}\NormalTok{.addLink(}\StringTok{"s5"}\NormalTok{, }\StringTok{"s2"}\NormalTok{)  }\CommentTok{\# s5{-}eth2 ↔ s2{-}eth4}
\end{Highlighting}
\end{Shaded}
\item
  \textbf{Non modificare \texttt{topologyslicing.py}}: il controller
  gestirà S5 con le regole di table-miss (invia al controller), ma noi
  installeremo le regole manualmente con \texttt{ovs-ofctl}.
\item
  Avviare controller e Mininet, verificare la connettività h1↔h3 (deve
  funzionare, S5 viene gestito con il comportamento di default).
\item
  \textbf{Terminare il controller} con Ctrl-C.
\item
  \textbf{Modificare manualmente le flow rule su S2} per redirigere il
  traffico attraverso S5:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Traffico in arrivo da s1 (eth1) → manda a s5 (eth3) invece che a s4 (eth2)}
\ExtensionTok{mininet}\OperatorTok{\textgreater{}}\NormalTok{ sh ovs{-}ofctl add{-}flow s2 in\_port=1,priority=10,actions=output:3}
\CommentTok{\# Traffico di ritorno da s5 (eth4) → manda verso s4 (eth2)}
\ExtensionTok{mininet}\OperatorTok{\textgreater{}}\NormalTok{ sh ovs{-}ofctl add{-}flow s2 in\_port=4,priority=10,actions=output:2}
\end{Highlighting}
\end{Shaded}
\item
  \textbf{Aggiungere flow rule su S5} per i tre scenari richiesti:

  \textbf{a) Tutto il traffico è permesso (forwarding semplice)}

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{mininet}\OperatorTok{\textgreater{}}\NormalTok{ sh ovs{-}ofctl add{-}flow s5 in\_port=1,priority=10,actions=output:2}
\ExtensionTok{mininet}\OperatorTok{\textgreater{}}\NormalTok{ sh ovs{-}ofctl add{-}flow s5 in\_port=2,priority=10,actions=output:1}
\end{Highlighting}
\end{Shaded}

  \textbf{b) Blocca i pacchetti ICMP} (ping non funziona, iperf TCP sì)

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{mininet}\OperatorTok{\textgreater{}}\NormalTok{ sh ovs{-}ofctl add{-}flow s5 icmp,priority=20,actions=drop}
\ExtensionTok{mininet}\OperatorTok{\textgreater{}}\NormalTok{ sh ovs{-}ofctl add{-}flow s5 in\_port=1,priority=10,actions=output:2}
\ExtensionTok{mininet}\OperatorTok{\textgreater{}}\NormalTok{ sh ovs{-}ofctl add{-}flow s5 in\_port=2,priority=10,actions=output:1}
\end{Highlighting}
\end{Shaded}

  \textbf{c) Blocca il traffico TCP} (iperf non funziona, ping sì)

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{mininet}\OperatorTok{\textgreater{}}\NormalTok{ sh ovs{-}ofctl add{-}flow s5 tcp,priority=20,actions=drop}
\ExtensionTok{mininet}\OperatorTok{\textgreater{}}\NormalTok{ sh ovs{-}ofctl add{-}flow s5 in\_port=1,priority=10,actions=output:2}
\ExtensionTok{mininet}\OperatorTok{\textgreater{}}\NormalTok{ sh ovs{-}ofctl add{-}flow s5 in\_port=2,priority=10,actions=output:1}
\end{Highlighting}
\end{Shaded}
...
\textbf{Comandi utili per l'esercizio B:}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{ovs-ofctl\ del-flows\ \textless{}sw\textgreater{}\ in\_port=\textless{}p\textgreater{}}
& Rimuove tutte le regole con quella porta in ingresso \\
\texttt{ovs-ofctl\ del-flows\ \textless{}sw\textgreater{}\ \textless{}proto\textgreater{}}
& Rimuove regole per protocollo (icmp, tcp, udp\ldots) \\
\texttt{ovs-ofctl\ add-flow\ \textless{}sw\textgreater{}\ \textless{}match\textgreater{},priority=\textless{}p\textgreater{},actions=\textless{}a\textgreater{}}
& Aggiunge una flow rule \\
\texttt{sudo\ tcpdump\ -i\ s5-eth1} & Sniffa pacchetti sull'interfaccia
virtuale di s5 \\
\end{longtable}
}

\begin{calloutnote}{Priority nelle flow rule}

OpenFlow sceglie la regola con \textbf{priorità più alta} tra quelle che
fanno match su un pacchetto. Impostare la regola di drop a priorità 20 e
il forwarding generico a 10 garantisce che i pacchetti ICMP (o TCP)
vengano bloccati prima di essere inoltrati. L'ordine di inserimento non
conta, conta solo la priorità numerica.
...
\tightlist
\item
  Come si implementa una service chain senza modificare il controller
  SDN?
\item
  Qual è la differenza tra un approccio di slicing basato su porte
  fisiche e uno basato su header di pacchetto (es. VLAN, IP)?
\item
  Perché nella topologia con S5 è necessario terminare il controller
  prima di installare le regole manuali?
\item
  Come si verificherebbe che i pacchetti TCP vengono effettivamente
  droppati da S5 e non semplicemente non raggiungono S4?
\end{itemize}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Passaggio dalla serie di Fourier alla trasformata: spettro discreto con Δf=1/T}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-01.jpg}}
\emph{Fig. --- Segnale periodico e relativo spettro discreto.
All'aumentare di T, le righe spettrali si avvicinano. Prendendo il
limite T→∞ si ottiene lo spettro continuo.}

\pandocbounded{\includegraphics[keepaspectratio,alt={Limite T→∞: lo spettro discreto converge verso uno spettro continuo}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-02.jpg}}
\emph{Fig. --- Quando T→∞, il segnale ``dimentica'' la propria
periodicità e lo spettro \textbar X\_D(f)\textbar{} diventa una funzione
continua della frequenza.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Spettro di s(t) per T=5, T=10, T=20: al crescere di T lo spettro si restringe}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-03.jpg}}
\emph{Fig. --- Spettri del segnale pulsato al variare della durata T.
All'aumentare di T il lobo principale della sinc si restringe,
riflettendo il principio di incertezza tempo-frequenza.}

\pandocbounded{\includegraphics[keepaspectratio,alt={Segnale pulsato con T=20 e il corrispondente spettro CFT}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-04.jpg}}
\emph{Fig. --- Segnale pulsato con T=20: il segnale nel dominio del
tempo (rettangolo di ampiezza 1 e durata 20) e il corrispondente spettro
S(f)=sin(20πf)/(πf).}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Applicazione di un filtro passa-basso per f\textless0.5: il segnale ricostruito è una versione smussata}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-05.jpg}}
\emph{Fig. --- Filtro passa-basso (soglia 0.5 Hz): lo spettro viene
troncato alle basse frequenze. La ricostruzione nel dominio del tempo
riproduce approssimativamente la forma del rettangolo, con
arrotondamento agli spigoli.}

\pandocbounded{\includegraphics[keepaspectratio,alt={Applicazione di un filtro passa-alto per f\textgreater0.5: rimangono solo le alte frequenze}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-06.jpg}}
\emph{Fig. --- Filtro passa-alto (soglia 0.5 Hz): rimangono solo le
componenti ad alta frequenza dello spettro. Il segnale ricostruito ha
una forma oscillatoria concentrata ai bordi del rettangolo originale.}

\pandocbounded{\includegraphics[keepaspectratio,alt={Applicazione di un filtro passa-banda per f∈{[}0.1, 0.5{]}}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-07.jpg}}
\emph{Fig. --- Filtro passa-banda (intervallo {[}0.1, 0.5{]} Hz):
vengono selezionate le componenti in una fascia di frequenze. Il
risultato è un segnale oscillante.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-01.png}}
\emph{Fig. --- Pipeline di conversione analogico-digitale e
...
\pandocbounded{\includegraphics[keepaspectratio,alt={FT di un segnale a banda limitata con larghezza B: lo spettro originale (blu) e le sue repliche dopo campionamento (verde)}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-08.jpg}}
\emph{Fig. --- Trasformata di Fourier di un segnale a banda limitata B.
Quando il segnale viene campionato, lo spettro si ``replica'' attorno a
multipli della frequenza di campionamento fs.}

\pandocbounded{\includegraphics[keepaspectratio,alt={Effetto della frequenza di campionamento sulla spaziatura delle repliche spettrali}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-09.jpg}}
\emph{Fig. --- Aumentando la frequenza di campionamento, le repliche
dello spettro si allontanano. Con campionamento sufficientemente veloce
(in basso), le repliche non si sovrappongono e il segnale originale è
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Aliasing: quando fs è troppo bassa, le repliche si sovrappongono e il contenuto in frequenza si mescola irreversibilmente}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-10.jpg}}
\emph{Fig. --- Aliasing per frequenza di campionamento insufficiente: la
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Condizione di Nyquist soddisfatta: le repliche non si sovrappongono quando fs=2B}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-11.jpg}}
\emph{Fig. --- Quando la condizione di Nyquist è soddisfatta (fs = 2B =
2fM), le repliche dello spettro si toccano esattamente senza
sovrapporsi: il segnale originale è recuperabile con un filtro ideale.}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Ampiezza della DFT per il segnale esempio: picco DC a n=0 e coefficienti simmetrici}]{images/lezione-27-lab-teoria-dei-segnali-trasformata-di-fourier-campionamento-e-dft-img-12.jpg}}
\emph{Fig. --- Spettro di ampiezza \textbar S\_n\textbar{} della DFT
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Grafico Condizione 1 -- Energia prodotta nell'intervallo {[}0,T{]} bounded tra due rette}]{images/lezione-28-kansal-problem-e-energy-neutrality-img-01.jpg}}
\emph{Fig. 1 --- La curva blu mostra l'energia prodotta cumulativa
\(E_T\) che oscilla tra le due rette tratteggiate
\(\rho_s T \pm \sigma\).}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Grafico Condizione 2 -- Consumo L\_T bounded dall'alto dalla retta rho\_c·T + delta}]{images/lezione-28-kansal-problem-e-energy-neutrality-img-02.jpg}}
\emph{Fig. 2 --- Il consumo cumulativo \(L_T\) rimane al di sotto della
retta \(\rho_c T + \delta\).}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Funzione utility vs duty cycle: piatta a zero, lineare, satura a u\_M}]{images/lezione-28-kansal-problem-e-energy-neutrality-img-03.jpg}}
\emph{Fig. 3 --- La funzione \(u(dc)\): segmento piatto a \(u_m\), rampa
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Esperimenti con Heliomote: (1) produzione per più giorni, (2) errore EWMA, (3) produzione su 70 giorni}]{images/lezione-28-kansal-problem-e-energy-neutrality-img-04.jpg}}
\emph{Fig. 4 --- Dall'alto: (1) profili di produzione giornaliera
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Diagramma Mermaid}]{images/mermaid-lezione-28-kansal-problem-e-energy-neutrality-01.png}}
\emph{Fig. 5 --- Architettura del sistema: il predittore stima la
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Grafico tempo di esecuzione vs numero di slot per Arduino, Raspberry PI e Linux x86}]{images/lezione-28-kansal-problem-e-energy-neutrality-img-05.jpg}}
\emph{Fig. 6 --- Tempo medio di esecuzione del codice C in funzione del
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Utility e livello medio di batteria in funzione del numero di slot, per mese}]{images/lezione-28-kansal-problem-e-energy-neutrality-img-06.jpg}}
\emph{Fig. 7 --- In alto: utility percentuale per mese al variare del
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Livello di batteria lungo il giorno in diversi mesi dell'anno}]{images/lezione-28-kansal-problem-e-energy-neutrality-img-07.jpg}}
\emph{Fig. 8 --- Andamento del livello di batteria nel corso della
...
\item
  ...
  per la neutralità energetica?
\item
  Descrivi la funzione di utility \(u(dc)\) e spiega come si calcolano i
  parametri \(\alpha\) e \(\beta\).
\item
  Descrivi l'algoritmo di Kansal nei due casi (sovraproduzione e
  sottoproduzione) e applica l'algoritmo a una tabella di produzione
  fornita.
\item
  Qual è il ruolo del filtro EWMA nella previsione della produzione
  energetica?
\item
  Cos'è un ``task'' nel modello task-based e perché il problema di
  ottimizzazione è NP-Hard?
\item
  Scrivi la ricorrenza di programmazione dinamica per il modello
  task-based e discutine la complessità.
\item
  Quali sono i limiti dell'algoritmo di Kansal rispetto al modello
  task-based?
\end{itemize}
...
\chapter{Esame Chessa --- Domande
Orali}\label{esame-chessa-domande-orali}
...
\section{Interoperabilità}\label{interoperabilituxe0-1}

\emph{Fig. --- Rete IoT eterogenea: dispositivi di produttori diversi
(colori diversi) comunicano attraverso gateway di integrazione.}
...
\emph{Fig. --- Architettura di sicurezza IoT: dispositivi vincolati (C),
...
\emph{Fig. --- Il paradigma publish/subscribe: i publisher inviano
messaggi al broker tramite PUBLISH; i subscriber si registrano
(SUBSCRIBE) e ricevono notifiche (PUBLISH dal broker); possono anche
disdire (UNSUBSCRIBE).}
...
\emph{Fig. --- Handshake a quattro fasi del QoS 2 tra MQTT client e
broker: PUBLISH → PUBREC → PUBREL → PUBCOMP.}
...
\subsection{Last Will \& Testament}\label{last-will-testament-1}
...
\emph{Fig. --- Sequenza di primitives tra Application Layer, Network
Layer e MAC Layer durante il processo di join di un dispositivo ZigBee.}
...
\emph{Fig. --- Stack protocollare ZigBee: dall'IEEE 802.15.4 (MAC) fino
...
\emph{Fig. --- Le tre topologie ZigBee: Star (sinistra), Tree (centro),
Mesh (destra). Nodo giallo = coordinatore, nodi blu = router, nodi rossi
= end-device.}
...
\emph{Fig. --- Albero di indirizzi ZigBee con \(R_m=2\), \(D_m=2\),
\(L_m=3\): ogni nodo gestisce un intervallo di indirizzi calcolato con
la formula \(C_{skip}\).}
...
\emph{Fig. --- Esempio di APS Binding Table: ogni entry mappa un
endpoint sorgente con cluster e destination address/endpoint.}
...
\emph{Fig. --- Esempio di binding ZigBee: Configuration tool configura
un on/off switch, che controlla una Simple lamp e un Dimmer switch, il
...
\emph{Fig. --- Codice Arduino che implementa il duty cycling selettivo
dei componenti (sensore, radio) e tabella dei consumi in mA per
componente e stato.}
...
\emph{Fig. --- Grafico log-log: vita della batteria (mesi) vs capacità
della batteria (mAh) per duty cycle 100\% (model1) e 5\% (model2).}
...
\emph{Fig. --- Modello di esecuzione Arduino: init → loop() ripetuto. I
...
\emph{Fig. --- Modello di esecuzione TinyOS: catena di eventi (Timer →
Read → task → Send) senza loop bloccante. Il data processing avviene nel
task asincrono.}
...
\emph{Fig. --- Esempio di programmazione con interrupt su Arduino:
...
\emph{Fig. --- S-MAC: i nodi A--F hanno schedule sincronizzati (verde =
...
\emph{Fig. --- B-MAC: il mittente (in alto) invia un lungo preamble
seguito dai data; il ricevitore (in basso) si sveglia, ascolta,
individua il preamble, rimane sveglio e riceve i data.}
...
\emph{Fig. --- Confronto LPL (B-MAC, righe superiori) e X-MAC (righe
...
\emph{Fig. --- Struttura del superframe IEEE 802.15.4: Beacon frame →
CAP (Contention Access Period, slot 0--6) → CFP (Contention Free Period,
...
\emph{Fig. --- Trasferimento dati indiretto in IEEE 802.15.4: il
dispositivo si sveglia al beacon, invia una Data request al
coordinatore, riceve ACK e poi i dati.}
...
\emph{Fig. --- Protocollo di associazione IEEE 802.15.4: sequenza di
messaggi tra NWK e MAC layer del dispositivo (sinistra) e del
coordinatore/router (destra).}
...
\emph{Fig. --- Grafico potenza/tempo: l'area tratteggiata in blu (Ps
...
\emph{Fig. --- Tabella con parametri di batteria per tre piattaforme
(RPI, Arduino, Tmote): tensioni min/max/ref, livelli di quantizzazione,
...
\emph{Fig. --- Sistema Kansal: il Device (con Scheduler e Tasks energy
model) è alimentato dall'Energy buffer (Battery + DC/DC converter),
rifornito dall'Energy harvester. L'Energy predictor stima la
disponibilità futura e informa lo Scheduler.}
...
\emph{Fig. --- Task model: lo Scheduler pianifica i task in base al
Tasks model (energia per task) e alle previsioni energetiche dell'Energy
...
\emph{Fig. --- Macchina a stati della Directed Diffusion: il nodo è Idle
...
\emph{Fig. --- GPSR greedy: S forwarda verso x (più vicino a D); x
incontra un void (nessun vicino è più vicino a D di x stesso) → attiva
il perimeter routing attorno al void per raggiungere D.}
...
\emph{Fig. --- Perimeter routing: il pacchetto naviga il confine del
void (obstacle rettangolare) passando per w→x→y→z→destination, con
alcuni nodi che rimandano indietro il pacchetto durante la face
traversal.}
...
\emph{Fig. --- Planarizzazione del grafo: GG (sopra) e RNG (sotto). Un
arco (u,v) è incluso solo se nessun nodo w cade nell'area tratteggiata
...
\chapter{Esame Orale --- Modulo 2}\label{esame-orale-modulo-2}
...
\section{OpenFlow Switch}\label{openflow-switch}
...
\subsection{Descrizione
dell'immagine}\label{descrizione-dellimmagine-17}
