#!/usr/bin/env python3
"""Concatenate normalized .md files into a single appunti.md."""
import re
...
def remove_obsidian_wikilink_images(text: str) -> str:
    # ![[image.jpg]] -> drop (no file available; only base64 was real content)
    return WIKILINK_IMG_RE.sub("", text)
...
def shift_headers(text: str, delta: int) -> str:
    """Shift all header levels by delta. Positive = demote, negative = promote."""
    if delta == 0:
        return text
...
def normalize(text: str, fallback_title: str) -> str:
    ...
    text = remove_obsidian_wikilink_images(text)
    ...
    if not levels:
        # No headers at all — just prepend H1
        return f"# {fallback_title}\n\n{text}"

    min_level = min(levels)
    # Promote everything so min_level becomes 1
    if min_level > 1:
    ...
    if h1_count == 0:
        ...
        # Demote everything by 1, prepend fresh H1
        text = shift_headers(text, +1)
        text = f"# {fallback_title}\n\n{text}"
    # if h1_count == 1, leave it
...
def title_from_filename(stem: str) -> str:
    # "Lezione 14 - IoT Design" -> "Lezione 14 — IoT Design"
    return stem.replace(" - ", " — ", 1)


def dedupe_by_lesson_number(paths):
    """If multiple files share the same lesson number (iCloud ' 2.md' dupes), keep shortest stem."""
    by_num = {}
    for p in paths:
...
def dedupe_esami(paths):
    """Drop iCloud ' 2.md' dupes for esami."""
    by_key = {}
    for p in paths:
...
esami = dedupe_esami(SRC.glob("Esame*.md"))
...
for p in esami:
    title = p.stem  # "Esame Chessa", "Esame Paganelli"
    body = normalize(p.read_text(encoding="utf-8"), title)
    parts.append(body.rstrip() + "\n\n```{=latex}\n\\newpage\n```\n\n")
