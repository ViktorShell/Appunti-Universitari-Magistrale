...
flowchart LR
    Client["Client Browser<br/>porta 80 / 443"] --> FE["Frontend<br/>Nginx<br/>(reverse proxy)"]
    FE -->|"/ → porta 8080"| App["Application Server<br/>backend"]
    App -->|"porta 1433"| DB["SQL Server<br/>database"]
    DB --- Vol[("Volume<br/>dati su host")]
...
%%{init: {"flowchart": {"useMaxWidth": true}}}%%
flowchart TD
    HW["Hardware fisico<br/>CPU · RAM · Scheda di rete (NIC) · Storage"] --> HV["Hypervisor"]
    HV --> VM1["VM 1<br/>Linux"]
    HV --> VM2["VM 2<br/>Windows"]
    HV --> VM3["VM 3<br/>..."]
    HV --> VS["Virtual Switch"]
    VS --> NIC["Scheda di rete fisica (NIC)<br/>uplink verso lo switch esterno"]
```
...
>
> - Qual è la differenza fondamentale tra container e VM in termini di isolamento?
> - Come funziona il meccanismo di cgroup per l'isolamento dei container?
> - Perché una vulnerabilità nel kernel Linux colpisce tutti i container ma non le VM?
> - Cos'è il virtual switch e perché è un componente critico della virtualizzazione?
> - Come funziona la live migration e perché è strategicamente importante?
> - Perché le CPU moderne hanno istruzioni hardware dedicate alla virtualizzazione?
> - Quali sono i trade-off di performance dell'uso di snapshot multipli su una VM?
> - Cosa accade allo storage quando si prende uno snapshot in una VM?
