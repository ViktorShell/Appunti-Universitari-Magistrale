...
flowchart LR
    A[Emulazione] -->|ogni istruzione interpretata| B[Lentezza 10-100×]
    C[Virtualizzazione] -->|istruzioni eseguite direttamente| D[Overhead ~10-15%]
    E[Stessa architettura host/guest] --> C
    F[Architetture diverse] --> A
```
...
%%{init: {"flowchart": {"useMaxWidth": true}}}%%
flowchart TD
    A[VM in esecuzione su Host A] --> B[Fase Pre-copy<br/>Copia bulk memoria + disco<br/>VM continua a girare]
    B --> C[Dirty page tracking<br/>Traccia pagine modificate<br/>durante la copia]
    C --> D{Dirty set<br/>abbastanza piccolo?}
    D -- No --> C
    D -- Sì --> E[Stop brevissimo della VM<br/>Copia CPU state + working set]
    E --> F[Riavvio VM su Host B]
    F --> G[Copia residui se necessario]
    G --> H[Cleanup: elimina VM su Host A]
    H --> I[VM operativa su Host B]
```
...
%%{init: {"flowchart": {"useMaxWidth": true}}}%%
flowchart LR
    SW[Switch fisico<br/>ARP cache: MAC→porta A] --> VA[vSwitch Host A]
    SW --> VB[vSwitch Host B]
    VA -->|traffico pre-migrazione| VM1[VM<br/>su Host A]
    VA -->|proxy forwarding<br/>durante transizione| VB
    VB --> VM2[VM<br/>su Host B]
    VB -->|gratuitous ARP| SW
    SW -->|ARP cache aggiornata| VB2[vSwitch Host B]
```
...
%%{init: {"flowchart": {"useMaxWidth": true}}}%%
flowchart LR
    subgraph C1[Cluster 1 — 8 nodi]
        N1[Host 1] --- N2[Host 2]
        N2 --- N3[Host 3]
        N3 --- N4[Host 4]
    end
    subgraph C2[Cluster 2 — 13 nodi]
        N5[Host 5] --- N6[Host 6]
        N6 --- N7[Host 7]
    end
    MGR[Management Layer<br/>SCVMM / vCloud Foundation] --> C1
    MGR --> C2
    STOR[Storage condiviso<br/>SAN o Hyper-converged] --> C1
    STOR --> C2
```
...
flowchart TD
    ...
    VS_A -->|UDP/IP VXLAN packet| PHY[Rete IP fisica<br/>layer 3]
...
>
> - Qual è la differenza tra emulazione e virtualizzazione? Quando conviene l'una e quando l'altra?
> - Descrivere le fasi della live migration e il meccanismo di dirty page tracking.
> - Come viene gestita la rete durante una live migration? Perché il packet loss è praticamente zero?
> - Perché i cluster di hypervisor hanno una dimensione massima tipica di 32 nodi?
> - Qual è la differenza tecnica tra infrastruttura hyper-converged e SAN? Vantaggi e svantaggi di ciascuna.
> - Cosa è VXLAN e perché è necessario nei datacenter moderni?
> - Perché i container non possono essere migrati "live" come le VM?
