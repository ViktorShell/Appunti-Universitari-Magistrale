...
flowchart TD
    subgraph Ethereum["Ethereum (chain privata)"]
        LP["LendingPool.sol<br/>(contratto principale, upgradeable)"]
        LC["LoanContract.sol<br/>(un'istanza per ogni prestito approvato)"]
        OC["BitcoinOracle.sol<br/>(on-chain oracle endpoint)"]
        LP -->|"deploy on approval"| LC
        LP -->|"legge saldo BTC"| OC
    end

    subgraph OffChain["Off-chain"]
        PY["oracle_service.py<br/>(legge blk.dat Bitcoin)"]
        SETUP["setup.py"]
        DEMO["demo.py"]
        BOT["contributor_bot.py"]
        GAS["gas_measurement.py"]
    end

    subgraph Bitcoin["Bitcoin Mainnet (blocchi 0–131000)"]
        BLK["blk.dat files<br/>(UTXO set)"]
    end

    PY -->|"requestUpdate() + update()"| OC
    BLK -->|"lettura blocchi"| PY
    SETUP -->|"deploy + fund accounts"| LP
    DEMO -->|"deposit/borrow/repay"| LP
    BOT -->|"event listener + vote"| LP
```
...
%%{init: {"flowchart": {"useMaxWidth": true}}}%%
flowchart TD
    S0["Step 0<br/>Setup ambiente Hardhat<br/>+ chain privata"] --> S1
    S1["Step 1<br/>BitcoinOracle.sol<br/>(on-chain)"] --> S2
    S2["Step 2<br/>oracle_service.py<br/>(off-chain + UTXO set)"] --> S3
    S3["Step 3<br/>LendingPool.sol<br/>deposit/withdraw/vote"] --> S4
    S4["Step 4<br/>LendingPool.sol<br/>proposte + risoluzione"] --> S5
    S5["Step 5<br/>LoanContract.sol<br/>rimborso + compensazione"] --> S6
    S6["Step 6<br/>Upgradeability<br/>UUPS proxy"] --> S7
    S7["Step 7<br/>setup.py + demo.py"] --> S8
    S8["Step 8<br/>contributor_bot.py"] --> S9
    S9["Step 9<br/>Gas measurement"] --> S10
    S10["Step 10<br/>Reentrancy vulnerability<br/>+ attack test"] --> S11
    S11["Step 11<br/>Test Hardhat completi"] --> S12
    S12["Step 12<br/>Report PDF"]
```
